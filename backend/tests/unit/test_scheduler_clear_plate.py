...
class TestPlateGateFarmDefaultWhenUnset:
    """Farm safety override: with no require_plate_clear row, the gate defaults ON.

    This keeps bed readiness fail-closed while an explicitly stored false value
    still disables the upstream confirmation gate.
    """
    ...
    async def test_check_queue_reads_plate_clear_setting_with_default_true(self, mock_pm, scheduler):
        """The farm queue check must pass default=True.

        This keeps a missing setting fail-closed and matches the fork schema/UI.
        """
        ...
        assert plate_calls[0].kwargs.get("default") is True, (
            "require_plate_clear must default True for the fail-closed farm policy"
        )
...
class TestPlateGateEndToEnd:
    ...
    @pytest.mark.parametrize(
        ...
        [
            (None, True),  # fresh install / never saved -> farm safety gate ON
