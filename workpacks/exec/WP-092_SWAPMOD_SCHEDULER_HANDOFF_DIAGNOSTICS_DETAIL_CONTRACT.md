# WP-092 SwapMod Scheduler Handoff Diagnostics Detail Contract
...
advertises the static contract for `diagnostics_summary.blocked_reason_details`.
API and future UI consumers can discover the detail object version and field
order without calling printer, scheduler dispatch, queue mutation, FTP, MQTT,
raw G-code, ERP, Obico, slicer, actuator, or physical bed boundaries.

## Why now

WP-091 added the source-qualified `blocked_reason_details` ledger. The next
smallest safe slice is exposing the detail contract through the existing status
route so clients can validate supported fields before rendering or asserting the
read-only diagnostics payload.
...
- Add additive status response fields for the blocked reason detail contract.
...
The existing diagnostics status response gains additive fields:

- `blocked_reason_details_contract_version`
- `supported_blocked_reason_detail_fields`

The supported detail field list is:
...
`harness-swapmod-scheduler-handoff-diagnostics-detail-contract` verifies the
focused no-network Makefile target exists and that mock services do not expose
detail-contract command, G-code, dispatch, printer-command, or bed-action routes.
...
- `make verify-fast`
- `python3 harness/scripts/check_workpack.py workpacks/exec/WP-092_SWAPMOD_SCHEDULER_HANDOFF_DIAGNOSTICS_DETAIL_CONTRACT.md`
- `git diff --check`
- Docker no-network Bambuddy app import smoke.

Evidence from this branch:

- RED check failed for the expected missing contract fields and focused Makefile
  target: missing `blocked_reason_details_contract_version`, missing
  `supported_blocked_reason_detail_fields`, and missing
...
- `make test-swapmod-scheduler-handoff-diagnostics` passed: harness 2 tests,
  backend 10 tests.
- `make verify-fast` passed: harness 111 tests in contract/unit phases and 2
...
No physical printer test is required for WP-092. This slice is a no-network,
...
No migration is added. Rollback is removing the additive status fields, focused
...
exposes static contract fields needed to diagnose and render blocked reason
...
- The focused detail-contract target passes.
- The status response reports `blocked_reason_details_contract_version=1`.
- The status response reports the supported detail fields in order.
