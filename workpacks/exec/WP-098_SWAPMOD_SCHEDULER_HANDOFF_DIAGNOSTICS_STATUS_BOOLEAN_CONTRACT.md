# WP-098 SwapMod Scheduler Handoff Diagnostics Status Boolean Contract
...
advertises which top-level status response fields are boolean values. API and
future UI consumers can discover status boolean fields without calling printer,
scheduler dispatch, queue mutation, FTP, MQTT, raw G-code, ERP, Obico, slicer,
actuator, or physical bed boundaries.

## Why now

WP-097 exposed summary collection fields. The next smallest safe slice is
exposing boolean fields on the status response itself so clients can separate
contract metadata, feature gates, and safety booleans from summary field
contracts.
...
- Add additive status response field `supported_status_boolean_fields`.
...
- `supported_status_boolean_fields`

The supported top-level status boolean field list is:
...
`harness-swapmod-scheduler-handoff-diagnostics-status-boolean-contract` verifies
the focused no-network Makefile target exists and that mock services do not
expose status-boolean-contract command, G-code, dispatch, printer-command, or
bed-action routes.
...
- `make verify-fast`
- `python3 harness/scripts/check_workpack.py workpacks/exec/WP-098_SWAPMOD_SCHEDULER_HANDOFF_DIAGNOSTICS_STATUS_BOOLEAN_CONTRACT.md`
- `git diff --check`
...
- RED check failed for the expected missing contract field: missing
  `supported_status_boolean_fields` in the API response and status handler.
- `make test-swapmod-scheduler-handoff-diagnostics-status-boolean-contract`
...
- `make test-swapmod-scheduler-handoff-diagnostics` passed: harness 2 tests,
  backend 10 tests.
- `make verify-fast` passed: harness 123 tests in contract/unit phases and 2
...
No physical printer test is required for WP-098. This slice is a no-network,
...
exposes static status boolean fields needed to diagnose and render safety state
consistently.
...
- The focused status-boolean-contract target passes.
- The status response reports the supported top-level status boolean fields in
  order.
