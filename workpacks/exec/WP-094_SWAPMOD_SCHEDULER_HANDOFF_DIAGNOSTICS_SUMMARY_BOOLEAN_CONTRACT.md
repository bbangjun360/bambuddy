# WP-094 SwapMod Scheduler Handoff Diagnostics Summary Boolean Contract
...
advertises which `diagnostics_summary` fields are booleans. API and future UI
consumers can discover the summary boolean field set without calling printer,
scheduler dispatch, queue mutation, FTP, MQTT, raw G-code, ERP, Obico, slicer,
actuator, or physical bed boundaries.

## Why now

WP-093 exposed the supported `gate_status` values. The next smallest safe slice
is exposing the summary boolean field set so clients can validate boolean state
without inferring it from examples or live diagnostics evaluations.
...
- Add additive status response field `supported_summary_boolean_fields`.
...
- `supported_summary_boolean_fields`

The supported summary boolean field list is:
...
not expose summary-boolean-contract command, G-code, dispatch, printer-command,
or bed-action routes.
...
- `make verify-fast`
- `python3 harness/scripts/check_workpack.py workpacks/exec/WP-094_SWAPMOD_SCHEDULER_HANDOFF_DIAGNOSTICS_SUMMARY_BOOLEAN_CONTRACT.md`
- `git diff --check`
...
- RED check failed for the expected missing contract field and focused Makefile
  target: missing `supported_summary_boolean_fields` and missing
  `harness-swapmod-scheduler-handoff-diagnostics-summary-boolean-contract`.
- `make test-swapmod-scheduler-handoff-diagnostics-summary-boolean-contract`
...
- `make test-swapmod-scheduler-handoff-diagnostics` passed: harness 2 tests,
  backend 10 tests.
- `make verify-fast` passed: harness 115 tests in contract/unit phases and 2
...
No physical printer test is required for WP-094. This slice is a no-network,
...
exposes static summary boolean fields needed to diagnose and render summary
state consistently.
...
- The focused summary-boolean-contract target passes.
- The status response reports the supported summary boolean fields in order.
