# WP-100 SwapMod Scheduler Handoff Diagnostics Status Version Contract
...
advertises which top-level status response fields are contract version numbers.
API and future UI consumers can discover status version fields without calling
printer, scheduler dispatch, queue mutation, FTP, MQTT, raw G-code, ERP, Obico,
slicer, actuator, or physical bed boundaries.

## Why now

WP-099 exposed top-level status string fields. The next smallest safe slice is
exposing contract version fields on the same status response so clients can
separate version metadata from booleans, strings, and summary field contracts.
...
- Add additive status response field `supported_status_version_fields`.
...
The supported top-level status version field list is:
...
`harness-swapmod-scheduler-handoff-diagnostics-status-version-contract` verifies
the focused no-network Makefile target exists and that mock services do not
expose status-version-contract command, G-code, dispatch, printer-command, or
bed-action routes.

## Validation commands

- `make test-swapmod-scheduler-handoff-diagnostics-status-version-contract`
...
- `make verify-fast`
- `python3 harness/scripts/check_workpack.py workpacks/exec/WP-100_SWAPMOD_SCHEDULER_HANDOFF_DIAGNOSTICS_STATUS_VERSION_CONTRACT.md`
- `git diff --check`
...
- RED check failed for the expected missing contract field: missing
  `supported_status_version_fields` in the API response and status handler.
- `make test-swapmod-scheduler-handoff-diagnostics-status-version-contract`
...
- `make test-swapmod-scheduler-handoff-diagnostics` passed: harness 2 tests,
  backend 10 tests.
- `make verify-fast` passed: harness 127 tests in contract/unit phases and 2
...
No physical printer test is required for WP-100. This slice is a no-network,
...
exposes static status version fields needed to diagnose and render contract
...
- The focused status-version-contract target passes.
- The status response reports the supported top-level status version fields in
  order.
