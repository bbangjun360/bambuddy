# WP-097 SwapMod Scheduler Handoff Diagnostics Summary Collection Contract
...
advertises which `diagnostics_summary` fields are collection/list values. API
and future UI consumers can discover collection summary fields without calling
printer, scheduler dispatch, queue mutation, FTP, MQTT, raw G-code, ERP, Obico,
slicer, actuator, or physical bed boundaries.

## Why now

WP-096 exposed nullable summary fields. The next smallest safe slice is exposing
summary collection fields so clients can render lists consistently without
inferring shape from sample diagnostics evaluations.
...
- Add additive status response field `supported_summary_collection_fields`.
...
- `supported_summary_collection_fields`

The supported summary collection field list is:
...
verifies the focused no-network Makefile target exists and that mock services do
not expose summary-collection-contract command, G-code, dispatch,
printer-command, or bed-action routes.
...
- `make verify-fast`
- `python3 harness/scripts/check_workpack.py workpacks/exec/WP-097_SWAPMOD_SCHEDULER_HANDOFF_DIAGNOSTICS_SUMMARY_COLLECTION_CONTRACT.md`
- `git diff --check`
...
- RED check failed for the expected missing contract field: missing
  `supported_summary_collection_fields` in the API response and status handler.
- `make test-swapmod-scheduler-handoff-diagnostics-summary-collection-contract`
...
- `make test-swapmod-scheduler-handoff-diagnostics` passed: harness 2 tests,
  backend 10 tests.
- `make verify-fast` passed: harness 121 tests in contract/unit phases and 2
...
No physical printer test is required for WP-097. This slice is a no-network,
...
exposes static summary collection fields needed to diagnose and render summary
state consistently.
...
- The focused summary-collection-contract target passes.
- The status response reports the supported summary collection fields in order.
