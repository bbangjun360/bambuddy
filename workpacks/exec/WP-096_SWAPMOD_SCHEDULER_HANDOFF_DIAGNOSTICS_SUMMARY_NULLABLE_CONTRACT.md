# WP-096 SwapMod Scheduler Handoff Diagnostics Summary Nullable Contract
...
advertises which `diagnostics_summary` fields may be `null`. API and future UI
consumers can discover nullable summary fields without calling printer,
scheduler dispatch, queue mutation, FTP, MQTT, raw G-code, ERP, Obico, slicer,
actuator, or physical bed boundaries.

## Why now

WP-095 exposed the summary count field set. The next smallest safe slice is
exposing the nullable summary field set so clients can distinguish absent
operator guidance from empty strings or unsupported fields.
...
- Add additive status response field `supported_summary_nullable_fields`.
...
The supported summary nullable field list is:
...
`harness-swapmod-scheduler-handoff-diagnostics-summary-nullable-contract` verifies
the focused no-network Makefile target exists and that mock services do not
expose summary-nullable-contract command, G-code, dispatch, printer-command, or
bed-action routes.
...
- `make verify-fast`
- `python3 harness/scripts/check_workpack.py workpacks/exec/WP-096_SWAPMOD_SCHEDULER_HANDOFF_DIAGNOSTICS_SUMMARY_NULLABLE_CONTRACT.md`
- `git diff --check`
...
- RED check failed for the expected missing contract field: missing
  `supported_summary_nullable_fields` in the API response and status handler.
- `make test-swapmod-scheduler-handoff-diagnostics-summary-nullable-contract`
...
- `make test-swapmod-scheduler-handoff-diagnostics` passed: harness 2 tests,
  backend 10 tests.
- `make verify-fast` passed: harness 119 tests in contract/unit phases and 2
...
No physical printer test is required for WP-096. This slice is a no-network,
...
exposes static summary nullable fields needed to diagnose and render summary
state consistently.
...
- The focused summary-nullable-contract target passes.
- The status response reports the supported summary nullable fields in order.
