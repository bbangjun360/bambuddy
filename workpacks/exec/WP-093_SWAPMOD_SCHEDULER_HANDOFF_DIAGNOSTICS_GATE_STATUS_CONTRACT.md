# WP-093 SwapMod Scheduler Handoff Diagnostics Gate Status Contract
...
advertises the supported `diagnostics_summary.gate_status` values. API and
future UI consumers can discover that diagnostics summaries report either
`allowed` or `blocked` without calling printer, scheduler dispatch, queue
mutation, FTP, MQTT, raw G-code, ERP, Obico, slicer, actuator, or physical bed
boundaries.

## Why now

WP-092 exposed the static detail-object contract for
`blocked_reason_details`. The next smallest safe slice is exposing the summary
status value set through the existing status route so clients can validate the
top-level diagnostics summary state before rendering or asserting a response.
...
- Add additive status response field `supported_gate_statuses`.
...
- `supported_gate_statuses`

The supported gate status list is:
...
`harness-swapmod-scheduler-handoff-diagnostics-gate-status-contract` verifies
the focused no-network Makefile target exists and that mock services do not
expose gate-status-contract command, G-code, dispatch, printer-command, or
bed-action routes.
...
- `make verify-fast`
- `python3 harness/scripts/check_workpack.py workpacks/exec/WP-093_SWAPMOD_SCHEDULER_HANDOFF_DIAGNOSTICS_GATE_STATUS_CONTRACT.md`
- `git diff --check`
...
- RED check failed for the expected missing contract field and focused Makefile
  target: missing `supported_gate_statuses` and missing
...
- `make test-swapmod-scheduler-handoff-diagnostics` passed: harness 2 tests,
  backend 10 tests.
- `make verify-fast` passed: harness 113 tests in contract/unit phases and 2
...
No physical printer test is required for WP-093. This slice is a no-network,
...
exposes static gate status values needed to diagnose and render summary state
consistently.
...
- The focused gate-status-contract target passes.
- The status response reports `supported_gate_statuses=["allowed", "blocked"]`.
